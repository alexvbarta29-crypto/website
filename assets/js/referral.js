/* =====================================================================
   Barta Window Washing — Referral program (refer.html + referred.html)
   ...
   (?t=TOKEN) and the friend landing's personalisation (?code=CODE).
   Request/response shapes: docs/REFERRAL-PROGRAM.md.
   ===================================================================== */
(function () {
  "use strict";
  ...
  /* The offer comes from <main>'s data attributes (sitedata.REFERRAL via
     build/referral_pages.py) so every dollar figure this script prints
  ...
  /* =================================================================
     refer.html — the referral form, success screen and private dashboard
     ================================================================= */
  ...
  if (form) {
    ...
    form.addEventListener("change", (e) => {
      if (e.target.name === "reward_pref") { rewardErr.hidden = true; rewardErr.textContent = ""; }
    });
    ...
    const validate = () => {
      ...
      if (!form.querySelector('input[name="reward_pref"]:checked')) {
        rewardErr.textContent = "Please choose your reward.";
        rewardErr.hidden = false;
    ...
    const fieldFor = (path) => {
      ...
      if (parts[0] === "referrer") {
        if (parts[1] === "reward_pref") return $("#ref-reward-credit");
    ...
    const showSuccess = (data, sent) => {
      ...
      const pref = (data.referrer && data.referrer.reward_pref) || sent.referrer.reward_pref;
      ...
      $("#ref-success-sub").textContent = "We’ll reach out to " + joinNames(names) + " with " + (many ? "their" : "the")
        + " " + money(cfg.friendOff) + " off, and you’ll get a " + prefLabel(pref) + " for each one who books.";
    ...
    form.addEventListener("submit", (e) => {
      ...
      const body = {
        referrer: {
          ...
          phone: trim(me.phone), email: trim(me.email), reward_pref: reward.value,
        },
    ...
    const referralItem = (x) => {
      ...
      let when = fmtDate(x.created_at);
      meta.textContent = x.status === "rewarded" && x.reward
        ? rewardText(x.reward) + (fmtDate(x.reward.issued_at) ? ", issued " + fmtDate(x.reward.issued_at) : "")
        : (when ? "Referred " + when : "");
  ...
  /* =================================================================
     referred.html — personalise the friend landing from ?code= (or ?ref=)
     ================================================================= */
  ...
  if (landingTitle) {
    ...
    // Every "claim" link on the page (hero button and the closing band)
    // carries src=referral; the code is added to each once it's known.
    const claimLinks = $$('a[href*="get-quote.html"]', main)
      .filter((a) => /src=referral/.test(a.getAttribute("href") || ""));
    const setClaim = (code) => claimLinks.forEach((a) => {
      const base = (a.getAttribute("href") || "").split("?")[0];
      a.setAttribute("href", base + "?" + (code ? "promo=" + encodeURIComponent(code) + "&" : "") + "src=referral");
    });
    const showCode = (code) => {
    ...
    const code = (params.get("code") || params.get("ref") || "").trim().toUpperCase();
      ...
      api(API + "?code=" + encodeURIComponent(code), { headers: { Accept: "application/json" } })
        .then(({ res, data }) => {
          if (res.ok && data && data.ok) {
            const name = data.referrer_first_name || "";
            const off = Number(data.friend_discount) || cfg.friendOff;
            if (name) {
              landingTitle.textContent = name + " sent you " + money(off) + " off";
              lead.textContent = name + " thinks you’ll love how we treat a home, and we’d love to prove it. "
                + "Your first service with Barta Window Washing is " + money(off) + " off, no strings attached.";
            }
            if (data.code) showCode(data.code);
          } else if (res.status === 404) {
            // Unknown code: the generic page, without a code that would
            // lead nowhere.
            codeWrap.hidden = true;
            setClaim("");
          }
          // Any other failure (no functions, network): the code from the
          // link stays on screen and travels on to the quote form as typed.
        })
        .catch(() => {});
    }
  }
