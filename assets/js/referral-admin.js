/* =====================================================================
   Barta Window Washing — Referral dashboard (admin/index.html)
...
(function () {
  ...
  const referrerText = (r) => {
    ...
    if (r.status === "rewarded" && r.reward) {
      return "Great news from " + cfg.biz + ": your " + rewardLabel(r.reward.type, r.reward.amount)
        + " for referring " + (r.first_name || "your friend") + " has been issued. Thanks for spreading the word!";
    }
    if (r.status === "completed" && r.reward && r.reward.type) {
      return "Thanks from " + cfg.biz + "! We have your pick of the " + rewardLabel(r.reward.type, r.reward.amount)
        + " for referring " + (r.first_name || "your friend") + ", and it’s on its way.";
    }
    if (r.status === "completed") {
      // The pick-your-reward text, verbatim (Twilio sends this one itself in SMS mode "all").
      return "Great news from " + cfg.biz + ": " + (r.first_name || "your friend") + "’s first service is complete, so your reward is ready! Pick a "
        + money(cfg.credit) + " account credit or a " + money(cfg.gift) + " gift card here: " + ((p0 && p0.status_url) || "");
    }
  ...
  const renderStats = () => {
    ...
    const owed = typeof s.rewards_owed === "number" ? s.rewards_owed : by.completed;
    const vals = {
      new: by.new, contacted: by.contacted, quoted: by.quoted, booked: by.booked, completed: owed,
      rewarded: by.rewarded, credit: money(s.credit_issued || 0), giftcards: s.gift_cards_issued,
    };
  ...
  const onStatusChange = (r, node, sel) => {
    ...
    act({ action: "set_status", id: r.id, status: next }, node).then((data) => {
      ...
      render();
      const who = fullName(r.first_name, r.last_name) || "Referral";
      toast(next === "completed"
        ? who + " marked " + statusLabel(next) + ". The referrer can now pick their reward; use “Text referrer” if texts aren’t automatic."
        : who + " marked " + statusLabel(next) + ".");
  ...
  const buildCard = (r) => {
    ...
    if (last) {
      ...
      const by = last.by === "office" ? "by the office" : last.by === "lead-form" ? "from the website" : "";
    ...
    if (r.reward && r.status === "rewarded") {
      ...
      hook(node, "reward").hidden = false;
      setText(node, "reward-text", r.reward && r.reward.type
        ? "They picked the " + rewardLabel(r.reward.type, r.reward.amount) + (r.reward.chosen_at ? " · " + fmtDate(r.reward.chosen_at) : "") + " · issue it below"
        : "Reward owed · waiting for their pick" + (r.reward_ready_at ? " since " + fmtDate(r.reward_ready_at) : ""));
    }
    ...
    const openPanel = (focusFirst) => {
      ...
      if (!radios.some((x) => x.checked)) {
        // What they picked on their tracking page wins; then the preference on file.
        const p = referrerOf(r);
        const pref = (r.reward && r.reward.type) || (p && p.reward_pref === "giftcard" ? "giftcard" : "credit");
  ...
  const renderList = () => {
    ...
    emptyEl.textContent = state.data.referrals.length
      ...
      : "No referrals yet. They'll show up here as customers send them from the program page.";
