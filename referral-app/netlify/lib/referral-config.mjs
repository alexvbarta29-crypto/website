...
// The app's own address, where the share links and the dashboard live. On
// Netlify the URL variable is the site's primary URL, so links come out
// right without any configuration; SITE_URL overrides it (previews, tests).
export const DEFAULT_SITE_URL = "https://refer.bartawindowwashing.com";
// The main website, for links out (services, quotes, legal pages).
export const MAIN_SITE_URL = "https://www.bartawindowwashing.com";
// BIZ.phone_display in build/sitedata.py — printed in the friend's text.
export const OFFICE_PHONE_DISPLAY = "(763) 314-3400";

export const STATUSES = Object.freeze(
  ["new", "contacted", "quoted", "booked", "rewarded", "declined"]);
// new/contacted/quoted are "pending" on the referrer's dashboard; booked and
// rewarded both count as booked there.
...
// The program page is the app root, so a tracking link is just /?t=TOKEN.
export const statusUrl = (token) => `${siteUrl()}/?t=${token}`;
export const adminUrl = () => `${siteUrl()}/admin/`;
...
export const SMS = Object.freeze({
  ...
  reward: ({ friend_first, reward }) =>
    `Great news from Barta Window Washing: ${friend_first} booked, so your ${reward} is ready. `
