...
import {
  REWARDS, STATUSES, PENDING_STATUSES, BOOKED_STATUSES, REWARD_TYPES, statusUrl, rewardAmount,
...
export function validateSubmission(body) {
  ...
  // The reward is picked once a friend's job is complete, so a preference
  // here is optional (older submissions and the office can still set one).
  const rewardPref = clean(r.reward_pref).toLowerCase();
  if (rewardPref && !REWARD_TYPES.includes(rewardPref))
    throw new ValidationError("The reward must be credit or gift card.", "referrer.reward_pref");
...
export function buildReferrer({ referrer, code, token, page, now }) {
  return {
    ...
    reward_pref: referrer.reward_pref || null,
    consent_at: now,
    source_page: page || "",
    created_at: now,
    updated_at: now,
...
export function dashboardTotals(referrals) {
  const t = { referred: referrals.length, booked: 0, rewarded: 0, pending: 0, to_choose: 0,
              credit_earned: 0, gift_cards_earned: 0 };
  for (const r of referrals) {
    if (BOOKED_STATUSES.includes(r.status)) t.booked++;
    if (r.status === "rewarded") t.rewarded++;
    if (PENDING_STATUSES.includes(r.status)) t.pending++;
    if (r.status === "completed" && !(r.reward && r.reward.type)) t.to_choose++;
    if (hasReward(r)) {
...
export const dashboardEntry = (r) => ({
  ...
  reward: isPlainObject(r.reward)
    ? { type: r.reward.type || null, amount: r.reward.amount || null,
        chosen_at: r.reward.chosen_at || null, issued_at: r.reward.issued_at || null }
    : null,
});
...
export function adminStats(referrals) {
  ...
  for (const r of referrals) {
    ...
    if (r.status === "completed") rewards_owed++;   // job done and paid, reward not yet issued
    if (hasReward(r)) {
...
export function referrerSummary(referrer, referrals) {
  ...
  return {
    ...
    booked: mine.filter((r) => BOOKED_STATUSES.includes(r.status)).length,
...
export const CSV_COLUMNS = [
  ...
  ["code", (r) => r.code],
  ["reward_ready_at", (r) => r.reward_ready_at],
  ["reward_type", (r) => (r.reward ? r.reward.type : "")],
  ["reward_amount", (r) => (r.reward ? r.reward.amount : "")],
  ["reward_chosen_at", (r) => (r.reward ? r.reward.chosen_at : "")],
