...
// SITE_URL is only overridden for previews; trailing slashes are stripped so
// the links below never come out as https://host//r/CODE.
export const siteUrl = () =>
  (process.env.SITE_URL || DEFAULT_SITE_URL).trim().replace(/\/+$/, "");
export const shareUrl = (code) => `${siteUrl()}/r/${code}`;
export const statusUrl = (token) => `${siteUrl()}/refer.html?t=${token}`;
