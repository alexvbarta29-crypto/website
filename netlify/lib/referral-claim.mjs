...
export async function recordClaim(store, { code, digits, lead, via = "claim", now = nowISO() }) {
  const how = via === "quote" ? "Requested a quote on the website" : "Claimed their discount on the referral page";
  ...
  let record = match;
  if (match) {
    // A listed friend usually arrives as a first name and a number; what
    // they type themselves fills in the blanks for the office.
    for (const k of ["last_name", "email", "address"])
      if (!match[k] && clean(lead && lead[k])) match[k] = clean(lead[k]);
  }
  if (match && (match.status === "new" || match.status === "contacted")) {
    ...
    match.history = [...(Array.isArray(match.history) ? match.history : []),
      historyEntry("quoted", "lead-form", how, now)];
    ...
    record = shareLinkReferral(referrer, digits, lead, now, how);
...
function shareLinkReferral(referrer, digits, lead, now, how) {
  const friend = {
  ...
  record.history.push(historyEntry("quoted", "lead-form", how, now));
  return record;
}
