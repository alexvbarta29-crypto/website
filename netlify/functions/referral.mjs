...
import { REWARDS, REWARD_TYPES, rewardAmount, rewardLabel, shareUrl, statusUrl } from "../lib/referral-config.mjs";
import {
  ...
  newestFirst, safeErr, clean, historyEntry,
...
export default async (req) => {
  if (req.method === "POST") {
    ...
    if (body && typeof body === "object" && body.action === "choose_reward") return chooseReward(body);
    return submit(body);
  }
...
async function submit(body) {
  ...
  if (referrer && referrer.code && referrer.token) {
    ...
    if (trusted) {
      if (sub.referrer.reward_pref) referrer.reward_pref = sub.referrer.reward_pref;
...
// ---------------------------------------------------------------------------
// POST { action: "choose_reward" } — the referrer picks their reward
// ---------------------------------------------------------------------------
// Only from the tracking page (the token proves it is them), only for a
// referral of theirs whose job the office has marked complete, and only
// until the office has issued it. Their pick also becomes the preference on
// file, so the next one defaults to it.
async function chooseReward(body) {
  const token = normalizeToken(body.t || body.token);
  const id = clean(body.id);
  const type = clean(body.reward_type).toLowerCase();
  if (!token) return fail(400, "That tracking link isn't valid.", "t");
  if (!/^r_[0-9a-z]+$/.test(id)) return fail(404, "Referral not found.", "id");
  if (!REWARD_TYPES.includes(type)) return fail(400, "Pick the credit or the gift card.", "reward_type");
  try {
    return await pickReward(await openStore(), token, id, type);
  } catch (err) {
    console.error(`referral choose_reward failed (${safeErr(err)})`);
    return fail(503, "The referral service is temporarily unavailable. Please try again in a minute.");
  }
}

async function pickReward(store, token, id, type) {
  const referrerId = await referrerIdForToken(store, token);
  ...
  const referral = await getReferral(store, id);
  if (!referral || referral.referrer_id !== referrer.id) return fail(404, "Referral not found.", "id");
  if (referral.status === "rewarded") return fail(409, "That reward has already been issued.");
  if (referral.status !== "completed") return fail(409, "That reward isn't ready to pick yet.");

  const now = nowISO();
  const changed = Boolean(referral.reward && referral.reward.type);
  referral.reward = {
    type, amount: rewardAmount(type), chosen_at: now, issued_at: null,
    note: (referral.reward && referral.reward.note) || "",
  };
  referral.history = [...(Array.isArray(referral.history) ? referral.history : []),
    historyEntry("completed", "customer", `${changed ? "Changed their pick to" : "Chose"} ${rewardLabel(type)}`, now)];
  referral.updated_at = now;
  referrer.reward_pref = type;
  referrer.updated_at = now;
  await saveReferralRecord(store, referral);
  await saveReferrer(store, referrer);
  console.log(`referral: reward chosen (${type}) for ${referral.id}`);
  return json({ ok: true, referral: dashboardEntry(referral) });
}
