...
// used by the office dashboard (public/admin/index.html):
//
...
import { smsMode, sendSms, e164, rewardReadyText } from "../lib/referral-notify.mjs";
...
async function act(store, req) {
  ...
  switch (action) {
    case "set_status": {
      ...
      const was = referral.status;
      // Moving a rewarded referral back means the reward was issued by
      // mistake; clearing it keeps the totals honest. The history keeps the
      // trail. Leaving "completed" for an earlier stage withdraws the offer
      // to choose, too.
      if (was === "rewarded" || (was === "completed" && status !== "completed")) referral.reward = null;
      ...
      if (status === "completed" && was !== "completed") {
        // The friend's first job is finished and paid: the reward is
        // unlocked. The referrer picks credit or gift card on their tracking
        // page; in SMS mode "all" they are told right away, otherwise the
        // dashboard offers the same text as a one-tap link.
        referral.reward_ready_at = now;
        if (!(referral.reward && referral.reward.type))
          referral.reward = { type: null, amount: null, chosen_at: null, issued_at: null, note: "" };
        const referrer = await getReferrer(store, referral.referrer_id);
        if (smsMode() === "all" && referrer && referrer.token) {
          referral.sms.reward_ready = await sendSms(e164(referrer.phone_digits),
            rewardReadyText({ friend: referral, token: referrer.token }));
          if (referral.sms.reward_ready) historyNote = [note, "Reward-ready text sent"].filter(Boolean).join(" — ");
        }
      }
      referral.history = [...history(referral), historyEntry(status, "office", historyNote, now)];
      referral.updated_at = now;
      await saveReferralRecord(store, referral);
      return json({ ok: true, referral });
    }
    case "issue_reward": {
      // Defaults to what the referrer picked on their tracking page, then to
      // the preference on file; the office can always override.
      let type = clean(body.reward_type).toLowerCase();
      if (!type && referral.reward && referral.reward.type) type = referral.reward.type;
      if (!type) {
        const referrer = await getReferrer(store, referral.referrer_id);
      ...
      referral.reward = { type, amount: rewardAmount(type),
        chosen_at: (referral.reward && referral.reward.chosen_at) || null, issued_at: now, note };
