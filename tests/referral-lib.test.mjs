...
import {
  REWARDS, SMS, STATUSES, siteUrl, shareUrl, statusUrl, rewardAmount, rewardLabel,
...
test("REWARDS mirrors build/sitedata.py REFERRAL", async () => {
  ...
  assert.deepEqual([...STATUSES], ["new", "contacted", "quoted", "booked", "rewarded", "declined"]);
});

test("site URLs default and strip trailing slashes", () => {
  delete process.env.SITE_URL;
  assert.equal(siteUrl(), "https://refer.bartawindowwashing.com", "the app's own default address");
  ...
  assert.equal(statusUrl("TOKEN"), "https://preview.example.com/?t=TOKEN");
  delete process.env.SITE_URL;
});
...
test("SMS templates match docs/REFERRAL-PROGRAM.md verbatim", () => {
  ...
  assert.equal(SMS.referrer({ n: 2, status_url: "https://www.bartawindowwashing.com/?t=T" }),
    "Thanks for referring 2 friend(s) to Barta Window Washing! You earn a $50 credit (or a $25 gift card) "
    + "for each one who books. Track your referrals: https://www.bartawindowwashing.com/?t=T");
  assert.equal(SMS.reward({ friend_first: "Jane", reward: "$50 credit" }),
    "Great news from Barta Window Washing: Jane booked, so your $50 credit is ready. Thanks for spreading the word!");
  assert.equal(SMS.office({ referrer_name: "Alex Barta", referrer_phone: "(763) 555-0100",
    ...
    + "https://www.bartawindowwashing.com/admin/");
});
...
test("record builders produce the contract shapes", () => {
  ...
  assert.deepEqual(Object.keys(referral).sort(), ["address", "code", "created_at", "duplicate_of", "email",
    ...
    "quote_requested_at", "referrer_id", "referrer_name", "referrer_phone", "reward", "rotor", "sms",
    "status", "updated_at"]);
...
test("dashboardTotals() and adminStats()", () => {
  ...
  assert.deepEqual(dashboardTotals(rs), { referred: 8, booked: 4, rewarded: 2, pending: 3, credit_earned: 50, gift_cards_earned: 1 });
  assert.deepEqual(adminStats(rs), {
    total: 8,
    by_status: { new: 1, contacted: 1, quoted: 1, booked: 2, rewarded: 2, declined: 1 },
    rewards_owed: 2, credit_issued: 50, gift_cards_issued: 1,
  });
  assert.deepEqual(dashboardTotals([]), { referred: 0, booked: 0, rewarded: 0, pending: 0, credit_earned: 0, gift_cards_earned: 0 });
});
