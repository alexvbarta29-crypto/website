...
test("mode all: friend, office, and referrer texts with the verbatim templates", async () => {
  ...
  assert.equal(referrer.params.get("Body"),
    `Thanks for referring 1 friend(s) to Barta Window Washing! You earn a $50 credit (or a $25 gift card) for each one who books. Track your referrals: https://www.bartawindowwashing.com/referral?t=${out.token}`);
...
// ---------------------------------------------------------------------------
// Job complete → the referrer is told to pick their reward
// ---------------------------------------------------------------------------
import { FAKE_ENV as ENV2, jsonRequest as request2 } from "./helpers/referral-harness.mjs";
const adminHandler = (await import("../netlify/functions/referral-admin.mjs")).default;
const markComplete = (id) => adminHandler(request2("/api/referral/admin", "POST",
  { action: "set_status", id, status: "completed" }, { Authorization: `Bearer ${ENV2.REFERRAL_ADMIN_KEY}` }));

test("mode all: marking the job complete texts the referrer the pick-your-reward link", async () => {
  process.env.REFERRAL_SMS_MODE = "all";
  const out = await post(submission());
  net.requests.length = 0;
  const res = await markComplete(out.friends[0].id);
  assert.equal(res.status, 200, await res.clone().text());
  const texts = net.twilio();
  assert.equal(texts.length, 1);
  assert.equal(texts[0].params.get("To"), "+17635550100");
  assert.equal(texts[0].params.get("Body"),
    "Great news from Barta Window Washing: Jane's first service is complete, so your reward is ready! "
    + `Pick a $50 account credit or a $25 gift card here: https://www.bartawindowwashing.com/referral?t=${out.token}`);
  const rec = record(out);
  assert.equal(rec.sms.reward_ready, true);
  assert.ok(rec.history.at(-1).note.includes("Reward-ready text sent"));
  // Marking it complete again (or moving it around) doesn't re-text.
  net.requests.length = 0;
  await markComplete(out.friends[0].id);
  assert.equal(net.twilio().length, 0);
});

test("mode office/off: marking the job complete sends nothing (the dashboard's one-tap link covers it)", async () => {
  for (const mode of ["office", "off"]) {
    process.env.REFERRAL_SMS_MODE = mode;
    const out = await post(submission({ referrer: { phone: mode === "off" ? "7635550300" : "7635550100" } }));
    net.requests.length = 0;
    await markComplete(out.friends[0].id);
    assert.equal(net.twilio().length, 0, mode);
    assert.equal(record(out).sms.reward_ready, undefined);
  }
});
