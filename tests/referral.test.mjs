...
const invalid = [
  ...
  ["missing reward_pref", submission({ referrer: { reward_pref: "" } }), "referrer.reward_pref"],
...
test("successful submission: response shape, headers, and stored records", async () => {
  ...
  assert.equal(out.status_url, `https://www.bartawindowwashing.com/refer.html?t=${out.token}`);
...
test("GET ?t=: totals math, masked friend names, no friend contact details", async () => {
  ...
  assert.deepEqual(out.totals, {
    referred: 4, booked: 2, rewarded: 1, pending: 1, credit_earned: 50, gift_cards_earned: 0,
  });
  ...
  assert.deepEqual(byName["Jane D."].reward,
    { type: "credit", amount: 50, issued_at: "2026-09-02T00:00:00.000Z" });
...
test("GET ?t=: gift-card rewards count as gift cards, not credit", async () => {
  ...
  assert.deepEqual(out.totals, {
    referred: 4, booked: 2, rewarded: 2, pending: 1, credit_earned: 50, gift_cards_earned: 1,
  });
});
