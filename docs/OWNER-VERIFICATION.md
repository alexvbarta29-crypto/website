...
| 20 | Business hours | Footer (every page), Contact page, LocalBusiness schema | `build/sitedata.py` → `BIZ["hours"]`, `build/schema.py` → `openingHoursSpecification` | "Mon–Fri 8am–6pm, Sat 8am–5pm, Sun Closed" | **CONFIRMED by owner (pre-launch correction)** | Owner corrected Saturday to close at 5pm. Footer text and `openingHoursSpecification` now agree: weekdays 08:00–18:00, Saturday 08:00–17:00, Sunday closed. |
