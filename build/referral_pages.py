...
# What a referred friend most often books first; the landing page shows
# these four so the offer is anchored to real work, not a generic pitch.
...
def _offer_attrs():
    ...
    return (f'data-friend-off="{FRIEND_OFF}" data-credit="{CREDIT}" data-gift="{GIFT}" '
            ...
            f'data-phone="{BIZ["phone_display"]}" data-email="{BIZ["email"]}"')
...
def _trust_list():
    return f"""<ul class="hero-trust">
          ...
          <li>{icon('home')} Family-owned in {BIZ['city']}</li>
        </ul>"""
...
def _reward_cards():
    """Credit vs gift card, as the same selectable cards the quote wizard
    uses for plan frequency (.promo-card.select-card), so the choice reads
    like the rest of the site. Credit is pre-selected: it is the option
    most customers pick, and a required radio group with nothing checked
    is a needless extra step."""
...
def _fallback():
    ...
    referral.js when the API is unreachable or answers 5xx), worded for a
    referral rather than a quote request."""
    ...
    <p><strong>We couldn&rsquo;t send that automatically.</strong> Please call or text us at
      ...
      and we&rsquo;ll add your referrals by hand, sorry for the trouble.</p>
...
def _referral_form(depth):
    ...
    return f"""<div class="ref-card">
      ...
      <div class="form-success ref-success" id="ref-success">
        ...
        <p class="ref-track" id="ref-track-wrap"><a id="ref-track" href="{root}refer.html">Track your referrals {icon('arrow')}</a><span class="ref-track-hint">Bookmark that private link: it shows each friend&rsquo;s progress and your rewards.</span></p>
...
def referral_page(depth=0, seo_title=None, schema=None):
    """Full HTML for refer.html: offer, how it works, the referral form (with
    ...
    html = C.head(
        title=seo_title(f"Refer a Friend, Give ${FRIEND_OFF} &amp; Get ${CREDIT}"),
        ...
        slug="refer.html", depth=depth, schema=schema, extra_head=css)
    html += C.nav(depth)
    crumbs = C.crumbs([("Home", root + "index.html"), ("Refer a Friend", None)])
  ...
  {C.cta_band(depth, heading="Know someone who needs us?",
              ...
              primary=("Refer a friend", "refer.html#refer-form"))}
</main>
{js}
{C.page_end(depth)}"""
    return html
...
def referred_page(depth=0, seo_title=None, schema=None):
    ...
    before (or without) JavaScript; referral.js only personalizes it with
    the referrer's first name and the code once GET /api/referral?code=
    answers. noindex: it is personal and thin as a search result."""
    seo_title = seo_title or (lambda s: s)
    root = C.rel(depth)
    css, js = _assets(depth)
    html = C.head(
    ...
    claim = f"get-quote.html?src=referral"
  ...
  <section class="phero rd-hero"><div class="container">
    ...
    <div class="rd-hero-inner">
      ...
      <p class="rd-code-wrap" id="rd-code-wrap" hidden><span class="rd-code-label">Mention code</span> <strong class="ref-code rd-code" id="rd-code"></strong> <span class="rd-code-hint">when we call, or use the button below and we&rsquo;ll fill it in for you.</span></p>
      <div class="phero-actions">
        <a class="btn btn-lg" id="rd-claim" href="{root}{claim}">Claim my ${FRIEND_OFF} off {icon('arrow')}</a>
  ...
  <section class="section-tight bg-mist"><div class="container">
    {C.trust_badges()}
    <p class="rd-how center">How the discount works: your ${FRIEND_OFF} comes straight off your first invoice. Mention the code when we quote you, or use the button and it&rsquo;s already filled in.</p>
  </div></section>
